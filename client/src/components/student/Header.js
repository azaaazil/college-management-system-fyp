...
const Header = () => {
  ...
  return (
    <div className="flex-[0.05] flex justify-between items-center mx-5 my-2">
      ...
      <h1 className="font-semibold text-white">Student Portal</h1>
      <div className="flex items-center space-x-3 text-[#CCCCCC]">
        <Avatar
          src={user.result.avatar}
          alt={user.result.name.charAt(0)}
          sx={{ width: 24, height: 24 }}
          className="border-blue-600 border-2 text-[#CCCCCC]"
        />
        <h1>{user.result.name.split(" ")[0]}</h1>
        <LogoutIcon
          onClick={logout}
          className="cursor-pointer hover:scale-125 transition-all text-[#CCCCCC]"
        />
      </div>
    </div>
  );
